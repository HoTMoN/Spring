...
* 			ModelAndView : 뷰와 뷰에 출력할 데이터정보를 저장(db에서 조회한 데이터를 이용해서 뷰를 만드는 경우 - response뷰)
...
public class IndexController {
	...
	//		/index.do로 요청하면 IndexController의 main메소드가 실행되도록 정의(path명은 중복이 불가능)
	@RequestMapping("/index.do")
	public String main() {
		System.out.println("어노테이션 기반 컨트롤러");
		return "test/index"; //view에 대한 정보 - WEB-INF폴더 빼고 확장자뺀 정보
	}
	...
	public ModelAndView test() {
		...
		return mav; //DispatcherServlet에서 알아서 response할 뷰에서 result데이터를 사용할 수 있도록 공유
					//요청재지정의 기본값은 forward
	}
}
